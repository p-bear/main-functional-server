pipeline {
  ...
  environment {
    ...
    repository = "pbear41/main-functional-server"
    dockerImage = ''
  ...
  stages {
    ...
    stage('get release properties file') {
      steps {
        sh 'cp /mnt/datadisk/data/properties/main-functional-server/application-release.yml ./src/main/resources/application-release.yml'
      }
    }
    stage('Gradle Build') {
      steps {
